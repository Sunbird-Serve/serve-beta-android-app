...
class StudentsPromotionDataSource(
    private val userType: String,
    private val digitalSchoolId: Int,
    private val grade: Int?,
    private val offeringId: Int?,
    private val courseProviderId: Int?,
    ...
    private fun getStudents(callback: LoadInitialCallback<Int, Student>) {
        ...
        searchJob = scope.launch {
            repo.getStudentsForPormotion(userType, courseProviderId,digitalSchoolId, grade, offeringId, filter,0)?.let {
                when (it) {
                    is Resource.Success -> {
                        it.data?.count?.let{
    ...
    private fun getMoreStudents(pageNo: Int, previousOrNextPageNo: Int, callback: LoadCallback<Int, Student>) {
        searchJob?.cancel()
        searchJob = scope.launch {
            repo.getStudentsForPormotion(userType, courseProviderId, digitalSchoolId, grade, offeringId, filter,pageNo)?.let {
                when (it) {
                    is Resource.Success -> {
