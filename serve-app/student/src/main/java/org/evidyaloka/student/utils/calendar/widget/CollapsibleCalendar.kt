...
import com.evidyaloka.student.utils.calendar.data.CalendarAdapter
import com.evidyaloka.student.utils.calendar.view.BounceAnimator
import com.google.firebase.crashlytics.FirebaseCrashlytics
import org.evidyaloka.student.R
import org.evidyaloka.student.utils.calendar.data.Day
import org.evidyaloka.student.utils.calendar.data.Event
...
class CollapsibleCalendar : UICalendar, View.OnClickListener {
    ...
    public var mAdapter: CalendarAdapter? = null
        ...
        if (mAdapter != null) {
            for (i in 0 until mAdapter!!.count) {
                ...
                if (isSelectedDay(day)) {
                    ...
                    txtDay.setBackgroundDrawable(unSelectedItemBackgroundDrawable)
                    unSelectedItemTextColor?.let {
                        txtDay.setTextColor(it)
                    }
                }
            }
        }
    ...
    override fun reload() {
        mAdapter?.let { mAdapter ->
            ...
            for (i in 0..6) {
                val view = mInflater.inflate(R.layout.calendar_week_day, null)
                val txtDayOfWeek = view.findViewById<View>(R.id.txt_day_of_week) as TextView
                txtDayOfWeek.setText("" + DateFormatSymbols().getShortWeekdays()[(i + firstDayOfWeek) % 7 + 1][0])
    ...
    fun getCurentWeek() : Int{
        return mCurrentWeekIndex
    }

    fun isNextWeekIsSameMonth(): Boolean {
        return (mCurrentWeekIndex + 1 < mTableBody.childCount)
    }
    fun isNextWeekHasSameMonth(): Boolean {
        return (mCurrentWeekIndex + 1 == mTableBody.childCount)
    }
    fun isPreWeekIsSameMonth(): Boolean {
        return !(mCurrentWeekIndex - 1 < 0)
    }
    fun isPreWeekHasPreMonth(): Boolean {
        return (mCurrentWeekIndex == -1)
    }
